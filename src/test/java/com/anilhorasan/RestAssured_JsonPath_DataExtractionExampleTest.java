...
public class RestAssured_JsonPath_DataExtractionExampleTest {
	...
	public void testJsonDataExtraction() throws Exception {
		...
		int courseCount = jsonPath.getList("courses").size();
		for (int i = 0; i < courseCount; i++) {
			String title = jsonPath.getString("courses[" + i + "].title");
			double price = jsonPath.getDouble("courses[" + i + "].price");
		...
		for (int i = 0; i < courseCount; i++) {
			String title = jsonPath.getString("courses[" + i + "].title");
			if ("RPA".equals(title)) {
				rpaCopies = jsonPath.getInt("courses[" + i + "].copies");
				rpaFound = true;
				break;
			}
		}
